...
class Ventana:
    ...
    def __init__(self):
        ...
        plt.subplots_adjust(bottom=0.220, top=0.820) #el top con 0.500 a 0.700
        self.grafica.set_xlim(-5.0,5.0)
        self.grafica.set_ylim(-5.0,5.0)
        self.fig.suptitle("Algoritmo MLP con Backpropagation")
        self.grafica_errores.set_title("Errores por Epoca")
        self.grafica_errores.set_xlabel("Época", color='C4')
        self.grafica_errores.set_ylabel("Error",color='C4')
        # Acomodo de los botones y cajas de texto  todos los botones que estan en la parte de abajo
        cordenadas_rango = plt.axes([0.210, 0.90, 0.07, 0.03])
        coordenadas_epcoas = plt.axes([0.420, 0.90, 0.07, 0.03])
        coordenadas_error_deseado = plt.axes([0.210, 0.85, 0.07, 0.03])
        coordenadas_capas=plt.axes([0.420, 0.85, 0.07, 0.03])
        coordenadas_pesos = plt.axes([0.510, 0.85, 0.125, 0.03])
        coordenadas_clase = plt.axes([0.510, 0.90, 0.125, 0.03])
        coordenadas_reiniciar = plt.axes([0.650, 0.85, 0.1, 0.03])
        coordenadas_entrenar_mlp= plt.axes([0.650, 0.90, 0.1, 0.03])
        axcolor = 'DarkKhaki'
        coordenadas_GD = plt.axes([0.450, 0.04, 0.16, 0.1],facecolor=axcolor)
        
        self.text_box_rango = TextBox(cordenadas_rango, "Aprendizaje: ")
        self.text_box_epocas = TextBox(coordenadas_epcoas, "Iteraciones: ")
        self.text_box_error_minimo_deseado = TextBox(coordenadas_error_deseado, "Error deseado: ")
        self.text_box_capas = TextBox(coordenadas_capas, "Neuronas por capa (): ")
        boton_pesos = Button(coordenadas_pesos, "Inicializar pesos")
        self.boton_clase = Button(coordenadas_clase, "Clase 1,2")
        boton_reiniciar = Button(coordenadas_reiniciar, "Reiniciar")
        boton_entrenar_mlp = Button(coordenadas_entrenar_mlp, "MLP")
        self.boton_GD = RadioButtons(coordenadas_GD, ('Gradiente Estocástico','Gradiente por Lotes'), activecolor='green')#hay que checar esto
    ...
    def cambio_clase(self,event):
        ...
        if(self.clase):
            self.boton_clase.label.set_text("clase 3,4")
        else:    
            self.boton_clase.label.set_text("clase 1,2")
    ...
    def reiniciar(self, event):
        ...
        self.texto_de_convergencia= None
        self.error_minimo=0.1
        self.neuronas_capa=None
